...
export default defineComponent({
    ...
    mounted() {
        ...
        this.$http.get(globaldata.markdown_assets.hostname + "markdown/" + finalPathType + "/" + this.codename + ".md")
            .then(function (response) {
                var received_content = response.data;
                if (parent_this.metatagsExist != false) {
            ...
            .catch(function (error) {
                console.error(error);
                parent_this.src = "# Ошибка загрузки контента. Проверьте ваше интернет-соединение.";
            });
